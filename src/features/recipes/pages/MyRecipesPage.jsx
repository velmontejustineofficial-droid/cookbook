...
export default function MyRecipesPage({ currentUserId, onBack }) {
  ...
  const ownRecipes = recipes.filter((recipe) => recipe.ownerId === currentUserId)
  ...
  return (
    <div className="w-full rounded-[28px] border border-[#eadfcd] bg-[#fffaf1] p-4 shadow-sm">
      ...
      {loading ? <p className="mt-5 text-sm text-[#756b5d]">Loading recipes...</p> : <div className="mt-5"><RecipeList recipes={filteredRecipes} onToggleFavorite={toggleFavorite} currentUserId={currentUserId} onEdit={setEditingRecipeId} onDelete={deleteRecipe} onView={setViewingRecipe} /></div>}
      <RecipeDetailsModal recipe={viewingRecipe} currentUserId={currentUserId} onClose={() => setViewingRecipe(null)} />
      {showCreate && <CreateRecipePage currentUserId={currentUserId} onBack={async () => { setShowCreate(false); await reload(); refresh((value) => value + 1) }} />}
      {editingRecipeId && <EditRecipePage recipeId={editingRecipeId} currentUserId={currentUserId} onBack={async () => { setEditingRecipeId(null); await reload(); refresh((value) => value + 1) }} />}
    </div>
  )
}